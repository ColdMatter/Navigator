...
namespace MOTMaster2.SequenceData
{
    ...
    public class Sequence
    {
        ...
        public Dictionary<string, object> CreateStepsDictionary(bool onlyEnabled = false)
        {
            Dictionary<string, object> dSteps = new Dictionary<string, object>();
            foreach (SequenceStep ss in Steps)
            {
                double timeMultiplier = 1.0; //TimebaseUnits.ms
                if (ss.Timebase == TimebaseUnits.us) timeMultiplier = 1e-3;
                else if (ss.Timebase == TimebaseUnits.s) timeMultiplier = 1000.0;
                double dur = ss.evalDuration();
                if (onlyEnabled)
                {
                    if (ss.Enabled) dSteps[ss.Name] = timeMultiplier * dur;
                }
                else dSteps[ss.Name] = timeMultiplier * dur;
            }
            return dSteps;
        }
