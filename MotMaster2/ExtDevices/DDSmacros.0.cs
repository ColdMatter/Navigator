...
namespace MOTMaster2.ExtDevices
{
    ...
    {    
        ...
        public object rampFreq()
        {
            ...
            // number of steps
            double smth = Utils.EnsureRange(intAgrs["smooth"], 0, 10);
            int nSteps = Convert.ToInt32(smth * 20e6 + 4000); // steps per sec
            double stepSize = 1 / (double)nSteps; // [sec]
            // rate
            int rate = Convert.ToInt32(stepSize*SysClk); 
            ls.Add("dcp " + sChn + " spi:DRR=0x" + DCPutils.dbl2hex(rate, 4) + DCPutils.dbl2hex(rate, 4));
        ...
        public object rampAmpl()
        {
            ...
            if (Utils.InRange(Convert.ToInt32(intAgrs["chn"]), 0, 1)) sChn = Convert.ToInt32(intAgrs["chn"]).ToString();
            // number of steps
            int nSteps = Convert.ToInt32(intAgrs["smooth"] * 100); // !!!
            // amplitudes
            long fullAmpl = (long)2147483647 * 2; // 100% from example p22
            double coeff = fullAmpl / 100.0;
            double amplStart = Convert.ToInt32(intAgrs["amplStart"]); double amplMiddle = Convert.ToInt32(intAgrs["amplMiddle"]);
            ls.Add("dcp " + sChn + " spi:DRL=0x" + DCPutils.dbl2hex(amplMiddle * coeff, 8) + DCPutils.dbl2hex(amplStart * coeff, 8));
            // rate
            int amplRange = Convert.ToInt32(Math.Abs(amplMiddle - amplStart));
            int rateUp = Convert.ToInt32(nSteps / intAgrs["durationUp"]); int rateDown = Convert.ToInt32(nSteps / intAgrs["durationDown"]);
            ls.Add("dcp " + sChn + " spi:DRR=0x" + DCPutils.dbl2hex(rateDown, 4) + DCPutils.dbl2hex(rateUp, 4));
            // step
            int stepDown = calcRampStep(1, rateDown, intAgrs["durationDown"], amplRange);
            int stepUp = calcRampStep(1, rateUp, intAgrs["durationUp"], amplRange);
            if (stepUp < 0 || stepDown < 0)
            {
                ErrorMng.errorMsg("Unable to calculate step in meta command <rampStep2>", 142);
                return ls;
        ...
        public object rampPhase()
        {
            ...
            ls.Add("dcp " + sChn + " spi:DRR = 0x" + DCPutils.dbl2hex(rateDown, 4) + DCPutils.dbl2hex(rateUp, 4));
            // step
            int stepDown = calcRampStep(4, rateDown, intAgrs["durationDown"], phaseRange);
            int stepUp = calcRampStep(4, rateUp, intAgrs["durationUp"], phaseRange);
            if (stepUp < 0 || stepDown < 0)
            {
                ErrorMng.errorMsg("Unable to calculate step in meta command <rampStep2>", 142);
                return ls;
