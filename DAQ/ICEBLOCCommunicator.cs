    ...
    public class ICEBLOCCommunicator
    {
        ...
        public ICEBLOCMessage CreateICEBLOCMessage(string json)
        {
           // Dictionary<string,object> rawMessage =  JsonConvert.DeserializeObject<ICEBLOCMessage>(json);
            Dictionary<string, ICEBLOCMessage> rawMessage = null;
            try
            {
                rawMessage = JsonConvert.DeserializeObject<Dictionary<string,ICEBLOCMessage>>(json);
            }
            catch (Exception e)
            {
                Console.WriteLine("Error in line:94 -> " + e.Message);
